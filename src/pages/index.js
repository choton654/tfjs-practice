...
export default function Home() {
  ...
  const loadAndRunModel = async (imgData) => {
    try {
      ...
      let resizeTensor = tf.image.resizeBilinear(imageTensor, [192, 192], true).toInt()
  ...
  return (
    <>
      <div style={{}}>
        ...
        {/* <img ref={imgRef} width={640} height={360}  crossOrigin src='https://storage.googleapis.com/jmstore/TensorFlowJS/Edx/standing.jpg' /> */}
        ...
        <button onClick={handelPlot}>try</button>
        {/* {points.length > 0 && (
          <>
            {points.map((p, i) => {
              const x = p[1] * 192
              const y = p[0] * 192
              return (
                <div key={i} style={{ position: 'absolute', left: x, top: y, height: 10, width: 10, backgroundColor: 'red', borderRadius: '50%' }} />
